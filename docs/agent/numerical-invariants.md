...
Both backends retain the legacy `0.0001`-mole numerical floor and transfer precision. Positive per-gas amounts below `0.01` mole remain real gas, with heat capacity and reaction participation; splitting or reconciling a mixture must not delete them merely because a component is smaller than a centimole. Existing mutators may zero values at or below the legacy floor. Analyzer display rounding is not state mutation.
